...
from v2.src.dqn_functions import check_precedence_feasibility, ssgs
from v2.src.replay_memory import Transition
from v2.conf import ELITS
...
class X:
    ...
    def _execute_solution(self, decisions: list[int]) -> tuple[State, bool, list[Transition]]:
        ...
        feasible: bool                = True
        _prev_lb: int                 = _current_solution.compute_lower_bound()
        _prev_ub: int                 = _current_solution.compute_upper_bound()
        for _, action_done in enumerate(decisions):
            ...
            if not feasible or task["Start"] <= 0:
                ...
                transitions.append(Transition(action=action_idx, previous_graph=_current_solution.graph, graph=_next_state.graph, delta_duration=_delta_duration, delta_lb=_delta_LB, lb=_next_lb, ub=_next_ub, dela_ub=_delta_UB, parent=transitions[-1] if transitions else None))
