...
from conf import INTERACTIVE, LR, NB_EPISODES
...
from src.dqn_functions import take_step, optimize_policy_net, optimize_target_net
...
def solve(path: str, instance_type: str, instance_name: str, interactive: bool):
    ...
    _saving_path: str          = f"{path}/best_sols/{instance_type}/{instance_name}/{instance_name}"
    _instance_path: str        = f"{path}/instances/{instance_type}/{instance_name}.RCP"
    ...
    EPS_DECAY                  = EPS_DECAY_RATE * NB_EPISODES * len(_tasks)
    ...
    for _episode in range(1, NB_EPISODES+1):
        ...
        for t in count():
            # ------------- ==*== SELECT AND SCHEDULE THE NEXT TASK ==*== -------------
            _e: float = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * _episode / EPS_DECAY) # probabiltiy to diversify
            if random.random() > _e and len(_REPLAY_MEMORY.flat_transitions) >= BATCH_SIZE:
                intensify      = True
            _action_idx: int   = multi_strategy_selection(state=_state, policy_net=_POLICY_NET, intensify=intensify, device=_device, memory=_TREE, current_transition=current_transition, next_transition_possibly_known=search_latter)
            _steps            += 1
            ...
            if _state.done:
                ...
                if _episode == NB_EPISODES:
                    ...
                    with open(_saving_path + "_result_gnn.txt", "w", encoding="utf-8") as file:
                        file.write(f"Best episode: {_best_episode}\nHuber Loss:{huber_loss:.2f}\nMakespan: {_best_state.make_span}\nSequence: {_best_state.id}")
                    _Cmax_TRACKER.save(_saving_path + "_makespan_gnn")
                    _LOSS_TRACKER.save(_saving_path + "_loss_gnn")
...
if __name__ == "__main__":
    ...
    _global_path = args.path if os.path.isabs(args.path) else os.path.join(os.path.dirname(os.path.abspath(__file__)), '')
    type=args.type
