...
def diversify(state: State, memory: ITree, t: Transition, device: str, possible_actions: list[dict], best_known_Cmax: int) -> Tensor:
    ...
    """
    unseen: list     = []
    p_ids: list[int] = [a["Id"] for a in possible_actions]
    random.shuffle(p_ids)
    for id in p_ids:
        _next_t: Transition = memory.search_transition(action=id, current_transition=t)
        if _next_t is None:
    ...
    if unseen:
        ...
        idx      = np.random.choice(min(TOP_K, len(unseen)))
        action   = unseen[idx][0]
...
def solve(path: str, instance_type: str, instance_name: str, interactive: bool):
    ...
    possible_actions           = find_feasible_tasks(_best_state.tasks, _best_state.scheduled_tasks)
    ...
    for _episode in range(1, NB_EPISODES+1):
        ...
        possible_actions                          = find_feasible_tasks(_state.tasks, _state.scheduled_tasks)
        ...
        for step in count():
            ...
            possible_actions       = find_feasible_tasks(_next_state.tasks, _next_state.scheduled_tasks)
            ...
            _transitions_in_episode.append(Transition(action=_action_idx, previous_graph=_state.graph, graph=_next_state.graph, lb=_next_lb, delta_lb=_delta_LB, ub=_next_ub, delta_ub=_delta_UB, delta_duration=_delta_duration, parent=_transitions_in_episode[-1] if _transitions_in_episode else None))
            _state                 = _next_state
            _prev_lb               = _next_lb
            _prev_ub               = _next_ub
