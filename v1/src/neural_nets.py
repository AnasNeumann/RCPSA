...
from v1.conf import O, P, D, R, S, EMBEDDING_DIMENSION, ATTENTION_HEADS, GNN_STACK_SIZE, DROPOUT_RATE
...
class HyperGraphGNN(Module):
    def __init__(self, task_features:int, resource_features:int, demand_features:int, d_model:int=EMBEDDING_DIMENSION, num_heads:int=ATTENTION_HEADS, num_layers:int=GNN_STACK_SIZE, attn_dropout: float=DROPOUT_RATE):
        ...
        for _ in range(num_layers):
            self.GAT_tasks_for_resources.append(GATConv(in_channels=(d_model, d_model), out_channels=d_model, heads=num_heads, concat=False, dropout=attn_dropout))
            ...
            self.GAT_resources_for_tasks.append(GATConv(in_channels=(d_model, d_model), out_channels=d_model, heads=num_heads, concat=False, dropout=attn_dropout))
